      ...
      <div class="form">
        <a-form class="form" ref="formRef" :model="formState" :rules="rules">
          <a-form-item class="form-item" label="用户名" has-feedback name="username">
            <a-input class="input" v-model.trim="formState.username" type="text" @keyup.enter="onSubmit()" placeholder="请输入用户名" autocomplete="off" allow-clear />
          </a-form-item>
          <a-form-item class="form-item" has-feedback name="password">
            ...
            <a-input class="input" v-model.trim="formState.password" type="password" @keyup.enter="onSubmit()" placeholder="请输入登录密码" autocomplete="off" allow-clear />
          </a-form-item>
    ...
    setup () {
      ...
      const onSubmit = () => {
        ...
        formRef.value.validate().then(() => {
          ...
          $store.dispatch('user/handleLogin', toRaw(formState)).then(res => {
            if (res.code !== '0000') {
